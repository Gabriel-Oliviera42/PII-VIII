...
namespace PII_VIII
{
    ...
    public class Usuario
    {
        ...
        {
            string query = $"UPDATE usuario SET nome = '{Nome}', email = '{Email}', datanascimento = '{DataNascimento.ToString("yyyy-MM-dd")}', altura ={Altura.ToString("0.00", CultureInfo.InvariantCulture)}, peso =  {Peso.ToString("0.00", CultureInfo.InvariantCulture)}, id_objetivo =  {IdObjetivo}, senha = '{Senha}', id_faixaetariapeso = {IdFaixa} WHERE id_usuario = {id}"; 
            con.Executar(query);            
        }
